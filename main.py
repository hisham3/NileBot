...
def course_repeating(context):
    ...
    if last_status['seats'] != info['seats'] and info['seats'] != []:
        context.bot.send_message(chat_id=CHAT_ID, text=f'{course}\n{"".join(text)}\nopened: {info["opened"]}')
...
def job(update, context):
    ...
    context.bot.send_message(chat_id=CHAT_ID, text='session logged in and working now...') #update.callback_query.message.chat_id
...
def cancel(update, context):
    ...
    course_class = context.user_data['course_class']
    course_class.quit()
    context.bot.send_message(chat_id=CHAT_ID, text='session ended.')
